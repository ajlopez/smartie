...
function ContractDefinition(name, is, body) {
	this.name = function () { return name; };
...
function FunctionDefinition(name, visibility, type, parameters, body) {
	this.name = function () { return name; };
	this.type = function () { return type; };
...
function Parser(text) {
    ...
    function parseContractDefinition() {
        ...
        return new ContractDefinition(name, is, new CompositeCommand(cmds));
    }
	...
	function parseFunctionDefinition() {
		...
		return new FunctionDefinition(name, visibility, type, parameters, body);
	}
    ...
    function parseIfCommand() {
        ...
        var cond = self.parseExpression();
        parseToken(TokenType.Punctuation, ')');
        var then = self.parseCommand();
        var elze = null;
        
        if (tryParseToken(TokenType.Name, 'else'))
            elze = self.parseCommand();
