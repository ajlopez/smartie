...
function LeftValue(name) {
	...
	this.compile = function (compiler) {
		return compiler.compileLeftValue(this);
	}
}
...
function CompositeCommand(commands) {
	...
	this.compile = function (compiler) {
		return compiler.compileCompositeCommand(this);
	};
}

function IfCommand(cond, then) {
	this.condition = function () { return cond; };
	this.then = function () { return then; };
	
	this.compile = function (compiler) {
		return compiler.compileIfCommand(this);
	};
}
...
function WhileCommand(cond, body) {
	...
	this.compile = function (compiler) {
		return compiler.compileWhileCommand(this);
	};
}

function BreakCommand() {    
    this.compile = function (compiler) {
        return compiler.compileBreakCommand(this);
    };
}

function ContinueCommand() {
    this.compile = function (compiler) {
        return compiler.compileContinueCommand(this);
    };
}
...
function ReturnCommand(expr) {
    ...
    this.compile = function (compiler) {
        return compiler.compileReturnCommand(this);
    };
}
...
function AssignmentExpression(name, expr) {
    ...
    this.compile = function (compiler) {
        return compiler.compileAssignmentExpression(this);
    };
}
...
function BinaryExpression(oper, left, right) {
	...
	this.compile = function (compiler) {
		return compiler.compileBinaryExpression(this);
	}
}
...
function Parser(text) {
	...
	this.parseCommand = function () {
		...
		if (token.type === TokenType.Name && token.value === 'if') {
			...
			return new IfCommand(cond, then);
		}
