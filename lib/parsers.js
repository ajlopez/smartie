...
function FunctionCommand(name, visibility, type, parameters, body) {
	this.name = function () { return name; };
	this.type = function () { return type; };
...
function Parser(text) {
	...
	function parseFunctionCommand() {
		...
		return new FunctionCommand(name, visibility, type, parameters, body);
	}
	...
	function parseParameters() {
		...
		if (token && !(token.type === TokenType.Punctuation && token.value === ')'))
			while (true) {
				...
				parameters.push(new Parameter(type, name));
